...
$sq = query("SELECT SUBSTRING(nick FROM 5) as nick, squadra, abil, status from ss_player");
$sa = array();
$saa = array();
$sta = array();
$sta2 = array();
...
$fd=array();
$res = query("SELECT nick, squadra, abil, status from ss_player where abil IS NULL");
while (($ra = mysql_fetch_assoc($res))) {
    $val=$ra["nick"];
    echo "<b>$val</b><br>";
    $sup="";
    $base = ccall('http://www.sharkscope.com/api/dalborgo/playergroups/sit_'.$val);
